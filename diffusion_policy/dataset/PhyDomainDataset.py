...
class PhyDomainDataset(BaseImageDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['action', 'agentview_image', 'robot0_eef_pos', 
                             'robot0_eef_quat', 'robot0_eye_in_hand_image', 'robot0_gripper_qpos'])
...
def test():
    ...
    hand = sample['obs']['robot0_eye_in_hand_image'].numpy() # (T, C, H, W)

    # 转回 matplotlib 显示格式 (T, H, W, C)
    agent_vis = np.moveaxis(agent, 1, -1)
    hand_vis = np.moveaxis(hand, 1, -1)
    ...
    for row, t in enumerate(frame_ids):
        ...
        axes[row, 1].imshow(np.clip(hand_vis[t], 0, 1))
        axes[row, 1].set_title(f'eye_in_hand t={t}')
        axes[row, 1].axis('off')
    ...
    if agent_vis.shape[0] >= 2:
        ...
        hand_diff = np.mean(np.abs(hand_vis[1] - hand_vis[0]))
        print(f"mean abs diff agentview t1-t0: {agent_diff}")
        print(f"mean abs diff eye_in_hand t1-t0: {hand_diff}")
        ...
        axes[1].imshow(np.abs(hand_vis[1] - hand_vis[0]))
        axes[1].set_title('eye_in_hand |t1-t0|')
        axes[1].axis('off')
