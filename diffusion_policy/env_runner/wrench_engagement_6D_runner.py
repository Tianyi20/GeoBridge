...
from WrenchEnv import WrenchEnv
...
class WrenchEngagement_6D_EnvRunner(BaseImageRunner):
    ...
    def _prepare_obs(self, obs):
        """Prepare env observation for a rotation-6D policy.

        The WrenchEnv observation still provides robot0_eef_quat in xyzw order.
        The 6D policy/dataset/task expect robot0_eef_rot6d instead, so we
        convert xyzw -> wxyz internally inside quat_xyzw_to_rot6d_np(), then
        wxyz quaternion -> rotation_6d using DP's RotationTransformer.

        Output keys are aligned with the 6D task shape_meta:
            agentview_image: float32, N/T..., C, H, W, range [0, 1]
            robot0_eef_pos: float32, (..., 3)
            robot0_eef_rot6d: float32, (..., 6)
            robot0_gripper_qpos: float32, (..., 1)
        """
        ...
        if 'robot0_eef_quat' in np_obs_dict:
            ...
            np_obs_dict['robot0_eef_rot6d'] = quat_xyzw_to_rot6d_np(quat_xyzw).astype(np.float32)
        else:
            raise KeyError(
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                action_6d = np_action_dict['action']

                action = action_rot6d_to_quat_xyzw_np(action_6d)
