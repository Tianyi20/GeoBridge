#!/usr/bin/env python3
"""Franka long-horizon gear deployment using a single-cycle policy.

Loads a checkpoint trained on PickUpEnv (`PhyDomain_AgentImage_workspace`)
and repeats the trained extraction cycle three times inside one episode:
...
def goto_sim_home(robot: FrankaClient):
    """Release the carried gear, move to SIM_HOME_JOINTS, restart impedance."""
    print('[Home] opening gripper before moving to sim home ...')
...
def preview_until_start(robot, env_cam, wrist_cam, keys: KeyMonitor, max_steps,
    ...
    while not keys.start_requested and not keys.quit_requested and not (
            stop_on_h and keys.stop_requested):
        if env_active:
            ec, _ = env_cam.get()
            if ec is not None:
                env_color = ec
...
def run_episode(policy, robot, env_cam, wrist_cam, keys: KeyMonitor,
                ...
                stop_on_cycle_requested: bool = False):
    ...
    record_enabled = recording is not None or episode_dir is not None
    if recording is None:
        recording = _new_recording()
    saved_actions = recording['actions']
    saved_states = recording['states']
    # Video captures one full-res frame per obs sample, per active camera.
    env_video_frames = recording['env_frames']
    env_video_t = recording['env_t']
    wrist_video_frames = recording['wrist_frames']
    wrist_video_t = recording['wrist_t']
    ...
    try:
        while step < max_steps and not keys.quit_requested:
            if keys.stop_requested:
                break
            if stop_on_cycle_requested and keys.cycle_requested:
                break
            if keys.pause:
                time.sleep(0.05)
    ...
    # ---- save ----
    if save_episode and episode_dir is not None:
...
def _new_recording():
    return {
        'actions': [], 'states': [], 'cycle_ends': [],
        'env_frames': [], 'env_t': [],
        'wrist_frames': [], 'wrist_t': [],
    }


def save_episode_recording(episode_dir: pathlib.Path, recording: dict):
    """Save all policy cycles as one long-horizon episode."""
    if not recording['actions']:
        return
    episode_dir.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(
        episode_dir / 'rollout.npz',
        actions=np.asarray(recording['actions']),
        robot_states=np.asarray(recording['states']),
        cycle_ends=np.asarray(recording['cycle_ends'], dtype=np.int64),
    )
    saved_video = False
    for fname, frames, ts_list in (
        ('agentview.mp4', recording['env_frames'], recording['env_t']),
        ...
        if len(frames) > 1:
            ...
            saved_video = True
    suffix = '' if saved_video else ' (no video frames)'
    print(f'[Episode] saved -> {episode_dir}{suffix}')
...
@click.option('-c', '--checkpoint', required=True, type=click.Path(exists=True),
              help='Path to .ckpt')
@click.option('-o', '--output_dir', default='data/FPSA_gear_long_horizon',
              type=click.Path(),
...
@click.option('--n_cycles', default=3,
              type=click.IntRange(min=1), show_default=True,
              help='Number of single-policy cycles in this long-horizon episode.')
@click.option('--no_save', is_flag=True, default=False)
@click.option('--auto_start', is_flag=True, default=False,
              help='Start the long-horizon episode immediately without waiting for S')
...
def main(checkpoint, output_dir, device, robot_ip, robot_port,
    ...
    try:
        if n_cycles:
            ...
            recording = None if no_save else _new_recording()

            print('\n[LongHorizon] resetting to sim home ...')
            keys.stop_requested = False
            goto_sim_home(robot)

            keys.start_requested = False
            keys.cycle_requested = False
            print('\n[LongHorizon] press S once to start the full episode; '
                  'H ends it immediately.')
            if not auto_start:
                preview_until_start(
                    robot, env_cam, wrist_cam, keys, max_steps,
                    stop_on_h=True,
                )

            for cycle in range(n_cycles):
                if keys.quit_requested or keys.stop_requested:
                    break
                keys.start_requested = False
                keys.cycle_requested = False
                print(f'\n[Cycle {cycle+1}/{n_cycles}] policy running; '
                      'press W when this cycle is complete ...')
                run_episode(
                    **run_kwargs,
                    episode_dir=None,
                    recording=recording,
                    save_episode=False,
                    stop_on_cycle_requested=True,
                )

                if keys.quit_requested or keys.stop_requested:
                    break
                if not keys.cycle_requested:
                    print('[LongHorizon] max_steps reached before W; ending episode.')
                    break
